...
test("validateProviderRuntimeConfig blocks unlisted external provider hosts", () => {
  const result = validateProviderRuntimeConfig(
    resolveSessionRuntimeAuthConfig({
      session_auth_mode: "anthropic-api-key",
      agent_api_protocol: "openai",
  ...
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /allowed provider list/);
});

test("validateProviderRuntimeConfig allows known external provider hosts", () => {
  const result = validateProviderRuntimeConfig(
    resolveSessionRuntimeAuthConfig({
      session_auth_mode: "anthropic-api-key",
      agent_api_protocol: "openai",
      agent_model: "mimo-v2.5-pro",
      agent_base_url: "https://token-plan-cn.xiaomimimo.com/v1",
      anthropic_api_key: "sk-test",
    }),
  );
...
test("validateProviderRuntimeConfig allows Azure OpenAI hosts", () => {
  const result = validateProviderRuntimeConfig(
    resolveSessionRuntimeAuthConfig({
      session_auth_mode: "anthropic-api-key",
